...
- Информация о исполнителях работы

- Имя текущего пользователя ОС

- Текущая рабочая директория

2) Системный вызов

- Запуск процесса notepad.exe через Process.Start()
...
- Рекурсивный вывод списка каталогов и файлов
...
- Метод Main переименован в "Register get_help get_help.GetTypesFromParentClass"

- Параметры метода имеют бессмысленные имена

2) Добавление мусорного кода

- Появились дополнительные операции (call DateTime.get_UtcNow)

- Добавлены фиктивные локальные переменные (V15, V16, V17)

3) Изменение структуры метода

- Увеличилось количество локальных переменных с 14 до 17

- Добавлены дополнительные операции в начале метода

4) Частичное шифрование строк

- Некоторые строковые константы остались читаемыми

- Отдельные строки подверглись преобразованию
...
- Метод Main имеет понятное имя и структуру

- Строковые константы читаемы: "Лабораторную работу выполнили...", "notepad.exe"

- Вызовы методов четко идентифицируемы: Console.WriteLine, Process.Start, Environment.UserName

- Локальные переменные имеют стандартные имена (V0, V1, V2...)

- Отсутствует мусорный код, логика программы прослеживается четко
...
Наглядное сравнение структуры приведено на рисунке
